...
)
{
    ...
    if (solution_.transient())
    {
        ...
        Info << "Total time for parcels injection = " << this->injectionCpuTime()
             << "s." << nl << endl;
...
void Foam::MomentumCloud<CloudType>::postEvolve()
{
    ...
    relevantTimes_.cacheFields(false);
...
void Foam::MomentumCloud<CloudType>::cloudReset(MomentumCloud<CloudType>& c)
{
    ...
    relevantTimes_.transfer(c.relevantTimes_);
...
:
    ...
    injectionCpuTime_(c.injectionCpuTime_),
    forces_(c.forces_),
    relevantTimes_(c.relevantTimes_),
