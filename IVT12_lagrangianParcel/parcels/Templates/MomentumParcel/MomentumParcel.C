...
)
{
    ...
    const typename TrackCloudType::relevantTimeType& relevantTimes = cloud.relevantTimes();
    ...
    const scalar minCo = cloud.solution().minCo();
    ...
    )
    {
        ...
        // Min fraction of the displacement to track in this loop.
        const scalar fmin = min(minCo, minCo/min(max(mag(s)/l, rootSmall), rootGreat));
        
        // The first parcel to arrive into a processor has null values permanently
        // assgigned to its continous phase. As these values are needed for the 
        // relevant time scales, they need to be updated beforehand.
        // The cause for that needs to be investigated
        if (ttd.rhoc() == 0 || ttd.muc() == 0)
        {
            // Update cell based properties
            p.setCellValues(cloud, ttd);
        }
        
        // stepFraction based on particle relevant times
        const scalar f1 = relevantTimes.calcRelevantTime(p, ttd, this->Re(ttd));
        
        f = min(f, f1);
        
        // minCo is the minimum fraction the parcel is allowed to move
        // except in the case where the use of minCo extrapolates the eulerian time step
        if ((p.stepFraction() + fmin) < 1)
        {
            f = max(f, fmin);
        }
